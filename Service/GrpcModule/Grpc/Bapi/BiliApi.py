...
from Service.GrpcModule.Grpc.Bapi.Constants import (
    URL_GAIA_GET_AXE,
    URL_BILI_MAIN_PAGE,
    URL_ABTEST_ABSERVER,
    URL_GET_WEB_AREA_LIST,
    URL_DYNAMIC_DETAIL,
    URL_SPACE_DYNAMIC,
    URL_RESERVE_RELATION_INFO,
    URL_LOTTERY_NOTICE,
    URL_GET_WEB_TOPIC,
    URL_REPLY_MAIN,
)
from Service.GrpcModule.Grpc.Bapi.RequestHandler import prepare_request_data
from Service.GrpcModule.Grpc.Bapi.BapiUtils import request_wrapper, appsign, gen_trace_id
...
async def get_web_topic(
    topic_id: int,
    request_conf: RequestConf = RequestConf(
        is_use_custom_proxy=True, is_use_available_proxy=False
    ),
):
    url = URL_GET_WEB_TOPIC
    params = {"topic_id": topic_id, "source": "Web"}
    cookie_data, headers = await prepare_request_data(request_conf)
    resp = await request_with_proxy_internal.request_with_proxy(
        method="GET",
        url=url,
...
async def get_lot_notice(
    ...
    request_conf: RequestConf = RequestConf(
        is_use_custom_proxy=True, is_use_available_proxy=False, is_use_cookie=True
    ),
):
    """
    获取抽奖notice
...
async def get_space_dynamic_req_with_proxy(
    ...
    dongtaidata = gen_dm_args(
        dongtaidata, gen_bili_web_cookie_params=cookie_data.ck.gen_web_cookie_params
    )  # 先加dm参数
    dongtaidata.update(
        {
            "x-bili-device-req-json": json.dumps(
            ...
            "x-bili-web-req-json": json.dumps(
                {"spm_id": spm_id}, separators=(",", ":")
            ),
        }
    )
    wbi_sign = await get_wbi_params(dongtaidata)
    dongtaidata.update({"w_rid": wbi_sign["w_rid"], "wts": wbi_sign["wts"]})
...
async def get_polymer_web_dynamic_detail(
    ...
    request_conf: RequestConf = RequestConf(
        is_use_custom_proxy=False, is_use_available_proxy=False, is_use_cookie=True
    ),
):
    ...
    cookie_data, headers = await prepare_request_data(
        request_conf,
        {
            ...
            "referer": (
                ...
                if dynamic_id
                else f"https://t.bilibili.com/{rid}?type={dynamic_type}"
            ),
        },
    )
    if dynamic_id:
    ...
    data.update({"w_rid": wbi_sign["w_rid"], "wts": wbi_sign["wts"]})
...
async def resource_abtest_abserver(
    ...
    brand,
    request_conf: RequestConf = RequestConf(
        is_use_custom_proxy=True, is_use_available_proxy=False, is_use_cookie=False
    ),
):
    ...
    params = {
        ...
        "statistics": json.dumps(
            {"appId": 1, "platform": 3, "version": app_version_name, "abtest": ""},
            separators=(",", ":"),
        ),
        "ts": int(time.time()),
    }
...
async def xlive_web_interface_v1_index_getWebAreaList(
    request_conf: RequestConf = RequestConf(
        is_use_custom_proxy=False, is_use_available_proxy=False
    )
):
...
if __name__ == "__main__":
    ...
    async def _test_get_lot_notice():
        resp = await get_lot_notice(business_type=10, business_id=5727405)
        print(resp)
