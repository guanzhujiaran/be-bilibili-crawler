...
class GetOthersLotDynRobot(UnlimitedCrawler[RobotTaskParams]):
    ...
    def __init__(self):
        ...
        self.SpareTime = (
            settings.get_others_lot.spare_time
        )  # 多少时间以前的就不获取别人的动态了
        ...
        self._phase: int = 1
        self._current_phase_objs: list[BiliSpaceUserItem | BiliDynamicItem] = []
        ...
        async def init_bili_space_user():
            if redis_data := await get_other_lot_redis_manager.get_target_uid_list():
                self.bili_space_user_items_set.update(
                    [
                        BiliSpaceUserItem(
                            uid=x.uid, lot_round_id=self.nowRound.lotRound_id
                        )
                        for x in redis_data
                    ]
                )
                ...
                self.bili_space_user_items_set.update(
                    [
                        BiliSpaceUserItem(uid=x, lot_round_id=self.nowRound.lotRound_id)
    ...
    async def main(self):
        ...
        # ---- 第一阶段：获取抽奖号的空间动态 ----
        self._phase = 1
        self._current_phase_objs: list[BiliSpaceUserItem] = list(
            self.bili_space_user_items_set
        )
        self.space_succ_counter = ProgressCounter()
